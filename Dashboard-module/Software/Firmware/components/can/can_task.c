...
/*
    * @brief:
    * Macro used to determine if the can packet received can be used to update
    * the display_data structure
*/
#define CAN_PACKET_FOR_DISPLAY_DATA(id) ((id == CAN_PACKET_ECU_1) || (id == CAN_PACKET_ECU_2) || (id == CAN_PACKET_PMU_1) || (id == CAN_PACKET_VDU))
////////////////////////////////////////////////////////////////////////////////
////                       LOCAL CONSTANTS                                  ////
////////////////////////////////////////////////////////////////////////////////
...
esp_err_t can_setup(twai_mode_t can_mode, twai_timing_config_t *can_speed, twai_filter_config_t *packet_filter_config)
{
    ...
    /* If everything went ok, exit with ESP_OK response */
    esp_response = ESP_OK;
    return esp_response;
}
...
void can_read(void *pvParameters)
{
    ...
    while(true)
    {
        ...
        switch (esp_response)
        {
            case ESP_OK:
            {
                #ifndef ENABLE_CAN_READ_DEBUG
                if (CAN_PACKET_FOR_DISPLAY_DATA(message.identifier))
                {
                    if( xSemaphoreTake( xSemaphore_display_data, ( TickType_t ) 10 ) == pdTRUE )
                    {
                        ...
                        switch(message.identifier)
                        {
                            ...
                            case CAN_PACKET_VDU:
                            {
                                display_data->tyre_pressure[0] = message.data[4];
                                display_data->tyre_pressure[1] = message.data[5];
                                display_data->tyre_pressure[2] = message.data[6];
                                display_data->tyre_pressure[3] = message.data[7];
                                break;
                            }

                            case CAN_PACKET_DAT:
                            {
                                general_status->time_hour = message.data[0];
                                general_status->time_minute = message.data[1];
                                general_status->time_second = message.data[2];
                                break;
                            }
                ...
                else if(CAN_PACKET_DAT == message.identifier)
                {
                    general_status->time_hour   = message.data[0];
                    general_status->time_minute = message.data[1];
                    general_status->time_second = message.data[2];
...
void can_send(void *pvParameters)
{
    ...
    while(true)
    {
        ...
        if(ESP_ERR_TIMEOUT == twai_transmit(&message, CAN_TRANSMIT_RATE))
        {
            ESP_LOGE("CAN_SEND", "Failed to transmit CAN message\n");
        };
